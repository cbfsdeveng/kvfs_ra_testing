...
int StatusToErrno(Status status) {
    switch (status) {
    case STATUS_OK: return 0;
    case STATUS_OBJECT_DOESNT_EXIST: return ENOENT;
    case STATUS_OBJECT_EXISTS: return EEXIST;
    case STATUS_INTERNAL_ERROR: return EIO;
    case STATUS_INVALID_PARAMETER: return EINVAL;
    default:
       LOG(ERROR) << "StatusToErrno: status " << status << " unexpected.";
        return EIO;
    }
}
