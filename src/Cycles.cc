...
Cycles::init() {
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (micros > 10000) {
                Cycles::cyclesPerSec = static_cast<double>(stopCycles - startCycles);
                Cycles::cyclesPerSec = 1000000.0*Cycles::cyclesPerSec/
                        static_cast<double>(micros);
                break;
            }
        }
        double delta = Cycles::cyclesPerSec/1000.0;
        if ((oldCycles > (Cycles::cyclesPerSec - delta)) &&
                (oldCycles < (Cycles::cyclesPerSec + delta))) {
            return;
        }
        oldCycles = Cycles::cyclesPerSec;
    }
}
